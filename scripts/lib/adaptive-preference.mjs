...
export function validateAdaptiveDatum(row) {
  ...
  const valuesValid = ['A', 'B'].every((side) => Number.isFinite(trial?.values?.[side]) && Math.abs(trial.values[side]) <= 5)
    && trial.values.A !== trial.values.B;
  const presentationValid = ['A', 'B'].every((side) => HASH.test(trial?.presentation?.[side])) && trial.presentation.A !== trial.presentation.B;
  const conditionValid = Number.isInteger(trial?.condition?.size) && trial.condition.size >= 8 && trial.condition.size <= 512 && ['light', 'dark'].includes(trial.condition.theme);
  const identityValid = trial?.repeatOf
    ? HASH.test(trial.repeatOf) && trial.id === trial.repeatId
    : trial?.id === trialId(trial.axis, trial.condition, trial.presentation.A, trial.presentation.B);
  if (!SAFE_ID.test(row.raterId) || !CHOICES.has(row.choice) || trial?.axis !== row.axis || JSON.stringify(trial.condition) !== JSON.stringify(row.condition) || !valuesValid || !presentationValid || !conditionValid || !identityValid) fail('Adaptive preference datum semantics are invalid.', 'adaptive-datum-invalid', 2);
  return true;
}
...
export function estimateIdealPoint(inputRows) {
  ...
  for (const row of rows) {
    const key = row.raterId.toLowerCase();
    const list = byRater.get(key) ?? []; list.push(row); byRater.set(key, list);
  }
  const sequenceLengths = new Set();
  for (const list of byRater.values()) {
    list.sort((a, b) => a.presentedIndex - b.presentedIndex);
    if (list.some((row, index) => row.presentedIndex !== index) || list.length < 5 || !list.at(-1).trial.repeatOf || list.at(-1).trial.repeatOf !== list[0].trial.id) fail('Ideal-point input contains an incomplete adaptive rater sequence.', 'ideal-point-sequence-incomplete', 2);
    sequenceLengths.add(list.length);
  }
  if (sequenceLengths.size !== 1) fail('Ideal-point rater sequences have different lengths.', 'ideal-point-sequence-incomplete', 2);
  ...
  for (const axis of [...new Set(rows.filter((row) => !row.trial.repeatOf).map((row) => row.axis))]) if (axes[axis].status !== 'ESTIMATED') reasons.push(`${axis}-axis-underpowered`);
  const core = {
    schemaVersion: 1,
    tool: MODEL_TOOL,
    nonAuthorizing: true,
